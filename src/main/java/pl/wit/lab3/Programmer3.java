...
public class Programmer3 extends Employee3{
	...
	public boolean matches(String firstName, String lastName, Date birthFrom, Date birthTo, Map<String,Short> mapProgrammingLanguages) {
		boolean isMPL = true;

		if (firstName == null && lastName == null && birthFrom == null && birthTo == null && mapProgrammingLanguages == null) {
			return true;
		}

		if (mapProgrammingLanguages != null) {
			for (String lang : mapProgrammingLanguages.keySet()) {
				if (!this.mapProgrammingLanguages.containsKey(lang) || this.mapProgrammingLanguages.get(lang) < mapProgrammingLanguages.get(lang)) {
					isMPL = false;
					break;
				}
			}
		...
		return super.matches(firstName, lastName, birthFrom, birthTo) && isMPL;
	}
